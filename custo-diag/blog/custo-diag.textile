...
Continuing from the example of our "previous blog post":http://melb.enix.org/the_tree_layout_secret on House of Cards, we'll see today how one can customize the elements of diagrams by using pictures.
...
If we set a workspace image on the _Influence_ diagram resulting from the "tree layout blog post":http://melb.enix.org/the_tree_layout_secret, we are able to see Frank Underwood's face instead of a simple white square.
